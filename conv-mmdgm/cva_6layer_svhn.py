...
def cva_6layer_svhn(learning_rate=0.01,
            n_epochs=600,
            dataset='svhngcn_var',
            batch_size=500,
            dropout_flag=1,
    ...
    logdir = 'results/supervised/cva/svhn/cva_6layer_'#+dataset+str(nkerns)+str(n_hidden)+'_'+str(weight_decay)+'_'+str(learning_rate)+'_'
    ...
    print 'cva_6layer_svhn', nkerns, n_hidden, seed, dropout_flag, drops, drop_p
    with open(logdir+'hook.txt', 'a') as f:
        print >>f, 'logdir:', logdir, 'predir', predir
        print >>f, 'cva_6layer_svhn', nkerns, n_hidden, seed, dropout_flag, drops, drop_p
    ...
    if super_predir is not None:
        ...
        pre_train = pre_train['model']
        for (para, pre) in zip(super_params, pre_train[:-2]):
            para.set_value(pre)
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        for minibatch_index in xrange(n_train_batches):          
            ttt, tpx, tpz, tqz, _z = train_model(minibatch_index)
            pxx+=tpx
            pzz+=tpz
            qzz+=tqz
            ...
            if (iter + 1) % validation_frequency == 0:
                print minibatch_index, 'stochastic training error', minibatch_avg_cost /float(minibatch_index), pxx/float(minibatch_index), pzz/float(minibatch_index), qzz/float(minibatch_index)#, 'z_norm', _____z/float(minibatch_index)
                with open(logdir+'hook.txt', 'a') as f:
                    print >>f, minibatch_index, 'stochastic training error', minibatch_avg_cost /float(minibatch_index), pxx/float(minibatch_index), pzz/float(minibatch_index), qzz/float(minibatch_index)#, 'z_norm', _____z/float(minibatch_index)

                print '1/10 time', time.clock() - tmp_start1
                tmp_start1 = time.clock()
                
                this_valid_losses = [valid_model(i) for i in xrange(n_valid_batches)]
                this_valid_score = np.mean(this_valid_losses)
                print epoch, minibatch_index, 'validation bound', this_valid_score
                #print tmp
                with open(logdir+'hook.txt', 'a') as f:
                    print >>f, epoch, minibatch_index, 'validation bound', this_valid_score
...
if __name__ == '__main__':
    ...
    cva_6layer_svhn(predir=predir, dropout_flag=dropout_flag, 

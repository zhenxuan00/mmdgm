...
class Pegasos_test:
    ...
    def __init__(self, rng, input, n_in, n_out, weight_decay, loss, std=1e-2):

        self.W = theano.shared(
            value = np.asarray(
                rng.normal(0, std, (n_in, n_out)),
    ...
    def objective(self, nc, y, y_matrix):
        ...
        return self.L2_reg + T.mean((T.max(l_y - label_result.T + self.linearfunction , axis = 1))**2)
    ...
    def hinge_loss(self, nc, y, y_matrix):
        ...
        l_y = (1-y_matrix)*self.loss
        label_result = T.tile(self.linearfunction[T.arange(y.shape[0]), y], (nc,)).reshape((nc,-1))
        return T.mean((T.max(l_y - label_result.T + self.linearfunction , axis = 1))**2)

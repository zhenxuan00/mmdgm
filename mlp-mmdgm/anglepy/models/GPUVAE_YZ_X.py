...
class GPUVAE_YZ_X(ap.GPUVAEModel):
    ...
    def factors(self, x, z, A):
        ...
        self.dist_qz['z'] = theanofunc([x['x'], x['mean_prior'], x['y']] + [A], [q_mean, q_logvar])
        ...
        if self.type_pz == 'gaussianmarg':
            logpz = -0.5 * (np.log(2 * np.pi) + ((q_mean-x['mean_prior'])**2 + T.exp(q_logvar))).sum(axis=0, keepdims=True)
    ...
    def gen_xz(self, x, z, n_batch):
        ...
        if x.has_key('x') and x.has_key('y') and not z.has_key('z'):
            ...
            q_mean, q_logvar = self.dist_qz['z'](*([x['x'], x['mean_prior'], x['y']] + [A]))
            _z['mean'] = q_mean
            _z['logvar'] = q_logvar

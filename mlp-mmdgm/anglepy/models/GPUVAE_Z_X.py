...
class GPUVAE_Z_X(ap.GPUVAEModel):
    ...
    def factors(self, x, z, A):
        ...
        if self.type_px == 'bernoulli':
            ...
            _logpx_normal = ap.logpdfs.normal2(x['x'][normal_list,:], x_mean[normal_list,:], x_logvar[normal_list,:])
            
            #print type(x_mean)
            
            _logpx_exponential = ap.logpdfs.exp(x['x'][exponential_list,:], x_mean[exponential_list,:])
    ...
    def init_w(self, std=1e-2):
        ...
        if len(self.n_hidden_p) > 0:
            ...
            if self.type_px == 'gaussian' or self.type_px == 'mixture' :
